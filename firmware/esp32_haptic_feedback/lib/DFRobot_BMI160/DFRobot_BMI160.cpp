...
int8_t DFRobot_BMI160::I2cGetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
  ...
  delay(10);
  // requestFrom()'s return value is how many bytes the device actually
  // acked back, not just how many were asked for — treat anything short as
  // a comm failure instead of blindly reading `len` bytes (Wire.read()
  // returns -1 per call once the buffer is empty, which previously got
  // reported to callers as valid sensor data).
  uint8_t received = Wire.requestFrom(dev->id, len);
  if (received != len) {
