...
def cv2_draw_depth(small, intensities, hazard_detected, severity, direction="none"):
    ...
    for zone, (x0, x1) in zone_spans.items():
        ...
        # Show every ACTIVE detector, not just the winner. The near-cluster
        # detector is labelled by SHAPE — T (thin/localized) vs N (broad near
        # surface) — because it fires on any close thing, wall included; a bare
        # "thin object" tag on a wall was misleading (live Pi finding).
        ...
        cv2.putText(frame,
                    f"s{p['sub']:.2f} w{p['wall']:.2f} f{p['f2w']:.2f} cov{bd['coverage']:.0%}",
                    (x0 + 8, view_h - 10),
    ...
    cv2.putText(frame, "active: T=thin object  N=near surface  W=blank-wall  F=floor-to-wall  (cov=cells firing)",
