...
class GStreamerParallelApp(GStreamerApp):
    def __init__(self, app_callback, user_data, parser=None):
        ...
        parser.add_argument(
            "--det-hef-path",
            default="yolov8n.hef",
            help="Specific HEF model to use for detection (default: yolov8n.hef)",
        )
    ...
    def get_pipeline_string(self):
        ...
        depth_sink = f"fakesink name=depth_sink sync=false"
        ...
        tracker_pipeline = TRACKER_PIPELINE(
            class_id=-1,
            kalman_dist_thr=0.8,      # Increased to allow for larger velocity predictions (default: 0.7)
            iou_thr=0.6,              # Lowered so fast objects with less overlap still match (default: 0.9)
            ...
            keep_lost_frames=8,        # 266ms grace for fast objects blurring out of YOLO (default: 2)
            name="det_tracker"
        )
        det_callback = USER_CALLBACK_PIPELINE(name="det_callback")
        det_sink = f"fakesink name=det_sink sync=false"

        # 3. Parallel tee architecture (display handled by cv2 in callbacks)
        pipeline_str = (
            f"{source_pipeline} ! tee name=t "
            f"t. ! {QUEUE(name='depth_branch_q')} ! {depth_pipeline_wrapper} ! {depth_callback} ! {depth_sink} "
            f"t. ! {QUEUE(name='det_branch_q')} ! {detection_pipeline_wrapper} ! {tracker_pipeline} ! {det_callback} ! {det_sink}"
        )
...
if __name__ == "__main__":
    hailo_logger.info("Starting Parallel Depth & Detection App V4...")
    main()
