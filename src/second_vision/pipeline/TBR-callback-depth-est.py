...
def depth_view_worker(view_queue):
    ...
    while True:
        try:
            ...
            continue
        small, intensities, hazard_detected, severity, hazard_dir, thin, thin_mask = payload
...
def on_depth_frame(element, buffer, user_data):
    ...
    if len(depth_mask) > 0:
        ...
        if len(depth_data.shape) == 2:
            ...
            # Hand the processor the CROPPED but full-resolution map: its
            # thin-structure pass must see cables/poles before the 64x48
            # decimation destroys them. It downsamples internally, so every
            # other detector sees exactly the same `small` grid as before.
            ...
            # The thin readings ride along so the HUD tags what the motors got,
            # rather than a weaker value recomputed from the 64x48 grid.
